...
function BottlesCalculator() {
  ...
  function calculateCombination(totalLiters: number, bottlesTypes: Array<number>) {
    const sortedBottles = bottlesTypes.sort((a: number, b: number) => b - a)
    let currentLiters: number = totalLiters
    const bottlesCombination = sortedBottles.reduce(
      (acc: BottlesCombination, curr: number) => {
        if (!currentLiters || curr > currentLiters) return acc
        const integerDivision = Math.floor(currentLiters / curr)
  ...
  function handleOnSubmit(event: { preventDefault: () => void }) {
    ...
    if (liters) calculateCombination(liters, [1, 3, 5, 15])
  }
      ...
      {bottles && (
        <div className="flex flex-col my-4 p-8 bg-gray-200 mx-auto rounded-md">
          ...
          <ul className="flex flex-col py-4 items-center bg-white list-disc rounded-md">
            {Object.entries(bottles)?.map(([key, value]: Array<string>) => (
